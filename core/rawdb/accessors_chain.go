...
func ReadHeaderRLP(db ethdb.Reader, hash common.Hash, number uint64) rlp.RawValue {
	...
	db.ReadAncients(func(reader ethdb.AncientReader) error {
		...
		//log.Debug("ReadHeaderRLP reader.Ancient", "data", data, "hash", hash, "crypto.Keccak256Hash(data)", crypto.Keccak256Hash(data), "number", number)
		if len(data) > 0 {
			header := new(types.Header)
			if err := rlp.Decode(bytes.NewReader(data), header); err != nil {
				//log.Error("ReadHeaderRLP Invalid block header RLP", "hash", hash, "err", err)
			} else {
				if header.Hash() == hash {
					//log.Debug("ReadHeaderRLP header.Hash()==hash true")
					return nil
				}
				//log.Debug("ReadHeaderRLP header.Hash()==hash false")
			}
		}
		// If not, try reading from leveldb
		data, _ = db.Get(headerKey(number, hash))
		//log.Debug("db.Get(headerKey(number, hash))", "number", number, "hash", hash, "data", data)
		return nil
	})
	return data
}
