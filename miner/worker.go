...
func (w *worker) taskLoop() {
	...
	for {
		select {
		case task := <-w.taskCh:
			log.Debug("taskLoop receive task:", "task", task)
...
func (w *worker) resultLoop() {
	...
	for {
		...
		case block := <-w.resultCh:
			...
			if block.AuxNumber().Sign() != 0 {
				sealhash.SetBytes(block.AuxExtra())
			} else {
...
func (w *worker) commitNewWork(interrupt *int32, noempty bool, timestamp int64) {
	...
	header := &types.Header{
		...
		AuxCoinAge:    big.NewInt(0),
		AuxDifficulty: big.NewInt(0),
		AuxNumber:     big.NewInt(0),
		AuxGasLimit:   big.NewInt(0),
		AuxGasUsed:    big.NewInt(0),
		AuxTime:       big.NewInt(0),
	}
